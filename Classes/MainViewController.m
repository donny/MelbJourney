...
- (void)viewDidLoad {
	...
	if (!headerView) {
		...
		headerLabel.text = @"Journey";
		
		UIImageView *headerImage = [[UIImageView alloc] initWithFrame:CGRectMake(13, 12, 26, 26)];
		headerImage.image = [UIImage imageNamed:@"hJourneyImage.png"];
...
- (UITableViewCell *)tableView:(UITableView *)aTableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (section) {
        ...
        if (content && indexPath.row < [content count]) {
			...
			NSString *originText = [item valueForKey:@"Origin"];
			NSString *destinationText = [item valueForKey:@"Destination"];
			NSRange range1, range2, subText;
			
			range1 = [originText rangeOfString:@"("];
			range2 = [originText rangeOfString:@")"];
			subText.location = range1.location + 1;
			subText.length = range2.location - range1.location - 1;
			
			cell.origin.text = [originText substringToIndex:range1.location];
			cell.originLocality.text = [originText substringWithRange:subText];
			
			range1 = [destinationText rangeOfString:@"("];
			range2 = [destinationText rangeOfString:@")"];
			subText.location = range1.location + 1;
			subText.length = range2.location - range1.location - 1;
			
			cell.destination.text = [destinationText substringToIndex:range1.location];
			cell.destinationLocality.text = [destinationText substringWithRange:subText];
			
            //cell.origin.text = [item valueForKey:@"Origin"];
            //cell.destination.text = [item valueForKey:@"Destination"];
        ...
        cell.origin.text = @"";
		cell.originLocality.text = @"";
        cell.destination.text = @"";
		cell.destinationLocality.text = @"";
    }
    return cell;
}
...
- (void)tableView:(UITableView *)aTableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.editing) {
        ...
        if (section) {
			...
			controller.originLoc = [item valueForKey:@"Origin"];
			controller.destinationLoc = [item valueForKey:@"Destination"];
            [self.navigationController pushViewController:controller animated:YES];
        }
    }
}
